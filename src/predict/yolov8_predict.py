...
# ==============================================================
    ...
    # for i, tile in tqdm(enumerate(raster_tiles), total=len(raster_tiles)):
        ...
        result = model(tile_path, conf=args.threshold, imgsz=544, device=device_name, verbose=False)
...
# ==============================================================
    ...
    polygons.to_file(os.path.join(args.output, 'output', os.path.splitext(rastername)[0] + "_YOLOv8_polygons.shp"),
                            crs=crs)
